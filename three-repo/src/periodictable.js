...
require("script-loader!./js/CSS3DRenderer.js")
...
class Periodictable {
  ...
  createTableVertices() {
    ...
    for (let i = 0; i < table.length; i += 5) {
      ...
      // [ clumn 18 ] 因为i + 3是所在列数， i + 4是行数， 所以可以通过计算得到位置
      object.position.x = table[i + 3] * 140 - 1500;
  ...
  createGrid(){
    let targets = []
    for (let i = 0; i < this.objects.length; i++) {
      let depth = Math.floor((i) / 25)
      let col = (i % 25) % 5
      let row = Math.floor((i % 25) / 5)
      const object = new THREE.Object3D();
      // 得到在圆上的位置
      object.position.x = col * 300 - 800;
      object.position.y = row * 300  - 800;
      object.position.z = depth * 200;
      targets.push(object)
    }
  ...
  createHelix(){
    ...
    let targets = []
    for (let i = 0; i < this.objects.length; i++) {
      let {x, y, z} = this.getHelixPosi(this.objects.length, i)
      const object = new THREE.Object3D();
      // 得到在圆上的位置
      object.position.x = x;
      object.position.y = y;
  ...
  createSphereVertices(){
    ...
    let targets = []
    for (let i = 0; i < this.objects.length; i++) {
      let {x, y, z} = this.getPosi(this.objects.length, i)
      const object = new THREE.Object3D();
      // 得到在球体上的位置
      object.position.x = x;
      object.position.y = y;
      object.position.z = z;
      // 将该向量与所传入的标量s进行相乘。 改变object的指向 
  ...
  getHelixPosi(objLength, i){
    // 先按圆来做
    var radius = 800;
    // 主要就是这个角度
    let phi = i * 0.213 + Math.PI;
    var x = radius * Math.sin( phi );
    var y = -( i * 8 ) + 450; 
  ...
  getPosi(objLength, i){
    // 球体表面的3维向量
    var radius = 800;
    // phi的角度是0~PI， 所以可以根据objLength来进行均分
  ...
  addLight() {
    ...
    let dirLight = new THREE.DirectionalLight(0x00ff00, 1);
