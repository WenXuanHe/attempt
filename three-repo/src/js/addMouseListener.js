...
export default (scene, renderer, camera, container) => {
    ...
    function handleRaycastersWrapper (event) {
    ...
    // canvas容器内鼠标点击事件添加
    container.addEventListener("touchend", handleRaycastersWrapper)
    container.addEventListener("mousedown", handleRaycastersWrapper);
